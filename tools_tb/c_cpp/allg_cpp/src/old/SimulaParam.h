...
#ifndef SIMULA_PARAM_H
#define SIMULA_PARAM_H
...
#include <sstream>
#include "SimulaDef.h"
#include "SimulaParamVar.h"
#include "SimulaParamInterpr.h"
#include "HlpString.h"
...
namespace Simula
{
  ...
  {
  private:
    std::vector<CParVarBase *> ParList;         // Liste der Parameter

    CParInterpr                ParInterpr;      // string-hierachie with parameter from file (interpretate string buffer from File)

    std::string                ErrText;          // Error-String
    status_type                Status;           // status: STATUS_OKAY, STATUS_NOT_OKAY

    std::size_t                maxLengthVarName; // maxima length of Variable names
    std::size_t                maxLengthUnit; // maxima length of Variable names
  ...
  public:
    ...
    // punit             unit if needed, if not punit = 0
    // pvar              PÜointer of Variable
    // allowdef          allow default value true/false
    // defvalue          Default value e.g.  "23"
    status_type declareVar(char *name, char *grouphierarchy, char *punit, int         *pvar,bool allowdef=false, char *defvalue=0);
    status_type declareVar(char *name, char *grouphierarchy, char *punit, double      *pvar,bool allowdef=false, char *defvalue=0);
    status_type declareVar(char *name, char *grouphierarchy, char *punit, std::string *pvar,bool allowdef=false, char *defvalue=0);

    // read  functions
    //================================================================================================================================
    status_type readParFile(char *fullfilename);

    // print Functions
    //================================================================================================================================
    // fullfilename         full file name to ParamList
    // allinst              true print all instances otherwise first
    status_type printParList(char *fullfilename,bool allinst);
 
    // setVal-Functions
    //================================================================================================================================
    // name              name of param-variable 
    // grouphierarchy    nested hierachy in string vector grouphierarchy[0] = "vehicle",grouphierarchy[1] = "body"
    // unit              unit if needed, if not ""
    // val               Value to set
    // instance          wich instance (first starts with 0, second 1, ...)
    status_type setVal(std::string &name, vec_string_t &grouphierarchy, std::string &unit, int         &val, std::size_t &instance);
    status_type setVal(std::string &name, vec_string_t &grouphierarchy, std::string &unit, double      &val, std::size_t &instance);
    status_type setVal(std::string &name, vec_string_t &grouphierarchy, std::string &unit, std::string &val, std::size_t &instance);
  private:
    // search-Functions
    //================================================================================================================================
    std::size_t searchName(std::string &name, vec_string_t &grouphierarchy);

    // compare-Functions
    //================================================================================================================================
    bool compareGroupHierarchy(vec_string_t &grouphierarchy0,vec_string_t &grouphierarchy1);

    // read  functions
    //================================================================================================================================
    status_type readRarFileStringBuffer(char *pfullfilename,std::string &buffer);
    status_type readParFileIntoString(std::string &fullfilename,std::string &buffer,std::size_t ibuffer); 

    // print-Functions
    //================================================================================================================================
    // grouphierarchy       actual nested hierachy in string vector empty is highest group level
    // numberofinstances    number of instances to plot
	  // ofs                  output files stream
	  status_type printParListVarAndGroup(vec_string_t &grouphierarchy,std::string &actgroup,s_t &numberofinstances,std::ofstream &ofs);
    status_type printParVarItem(CParVarBase *pvar,s_t &nhierarchie,s_t &nvarnamelen,s_t &nunitlen,s_t &numberofinstances,std::ofstream &ofs);
    status_type printParVarItemInt(CParVar<int> *pvar,s_t &numberofinstances,std::ofstream &ofs);
    status_type printParVarItemDouble(CParVar<double> *pvar,s_t &numberofinstances,std::ofstream &ofs);
    status_type printParVarItemString(CParVar<std::string> *pvar,s_t &numberofinstances,std::ofstream &ofs);
    status_type printParGroupItem(std::size_t nhierarchie,std::string &group,std::ofstream &ofs);
    // delete-Functions
    //================================================================================================================================
    void deleteParList(void);

    // get-Functions
    //================================================================================================================================
    s_t  getMaxInstancesOfParList(void);
    void getListOfGroupFromParList(/*input*/vec_string_t &grouplistabove, /*output*/vec_string_t &list_subgroups);
    void getListOfVarFromParList(/*input*/vec_string_t &grouplist, /*output*/vec_uint_t  &ui_liste_of_var);
