...
namespace slf
{
  ...
  -----------------------------------------------------------------------*/
	  ...
	  size_t kp1, m, nm1, k, ier;
	  double t;
    size_t i,j;

	  ier = 0;
    if (ip.size() < n) ip.resize(n);
	  ip[n-1] = 1;
	  if (n != 1) 
    {
		  nm1 = n - 1;
	  ...
	  if (n != 1) 
    {
		  ...
		  if (lb != 0) 
      {
			  for (size_t k = 0; k < nm1; k++) 
        {
				  ...
				  br[k] = tr;
				  bi[k] = ti;
				  for (size_t i = kp1; i < SLF_MIN(n, lb+k+1); i++) 
          {
					  prodr = AR[i][k]*tr - AI[i][k]*ti;
					  prodi = AI[i][k]*tr + AR[i][k]*ti;
					  br[i] += prodr;
  ...
  -----------------------------------------------------------------------*/
	  ...
	  size_t md, md1, mdl, ju, nm1, k, kp1, m, mm, jk, ijk, ier, i, j;
	  double t;
	  ...
	  if ((n != 1) && (ml != 0)) 
    {
		  ...
		  for (k = 0; k < nm1; k++) 
      {
			  kp1 = k + 1;
			  m = md;
			  mdl = SLF_MIN(ml, n - k - 1) + md;   
			  for (size_t i = md1; i <= mdl; i++) { 
				  if (fabs(A[i][k]) > fabs(A[m][k])) m = i;
			  }
			  ip[k] = m + k - md;
			  t = A[m][k];
			  ...
			  if (t == 0.0) 
        {
			  ...
			  t = 1.0/t;
			  for (i = md1; i <= mdl; i++) A[i][k] *= -t; 
			  ju = SLF_MIN(SLF_MAX(ju, mu+ip[k]+1), n);
			  mm = md;
			  if (ju >= kp1) 
        {
				  for (j = kp1; j < ju; j++) 
  ...
  -----------------------------------------------------------------------*/
	  ...
	  size_t m, kb, nm1, md, md1, mdl, mdm, imd, kmd, lm;
	  double t;
  ...
  -----------------------------------------------------------------------*/	  
	  ...
	  size_t i,kp1, m, nm1, k, md, md1, mdl, mm, jk, ijk, ju, ier, j;
	  double tr, ti, den, prodr, prodi;
	  ...
	  if ((n != 1) && (ml != 0)) 
    {
		  ...
		  for (k = 0; k < nm1; k++) 
      {
        ...
        { 
			  ...
			  ju = SLF_MIN(SLF_MAX(ju, mu+(size_t)ip[k]+1), n); 
			  mm = md;
			  if (ju >= kp1) 
        {
				  for (j = kp1; j < ju; j++) 
  ...
  void solbc(const size_t n, CMatrixD &AR, CMatrixD &AI, size_t ml, size_t mu,
    CVectorD &br, CVectorD &bi, CVectorI &ip)
  {
    solbc(n, AR, AI, ml, mu, &br[0], &bi[0], ip);
  }
  void solbc(const size_t n, CMatrixD &AR, CMatrixD &AI, size_t ml, size_t mu,
  ...
  -----------------------------------------------------------------------*/
	  ...
	  size_t nm1, m, kb, md, md1, mdl, mdm, imd, kmd, lm;
	  double den, prodr, prodi, tr, ti;
  ...
  ------------------------------------------------------------------*------*/
	  ...
	  size_t ii, la, kp1, mm1, mp1, i, j;
	  double x, y;

	  la = igh - 2;
	  kp1 = low + 1;
	  if (la < kp1) return;
	  for (size_t m = kp1; m <= la; m++) 
    {
		  mm1 = m - 1;
		  x = 0.0;
		  ii = m;
