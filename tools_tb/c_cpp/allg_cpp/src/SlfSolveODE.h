...
{ 
    ...
    typedef okay_t (*FcnEqJac)(const double &tact, const CVectorD &x, CMatrixD &dxp_dx, size_t ldsp_dx);
    typedef okay_t (*FcnEqMas)(CMatrixD &am, size_t lmas);
}
...
namespace slf
{
  ...
  //typedef void (*Jac_fp)(const int32_t &n, const double &tact, const CVectorD &x, int32_t &ml, int32_t &mu, CMatrixD &dxp_dx, size_t ldsp_dx);
  // dummy
  //typedef int (*S_fp)(...);
  ...
  {                                    // method switching stiff/nonstiff
    ...
    size_t ijac;            // (1)  0/1  Jakobimatrix berechnen
    size_t mljac;           // (0)  <= n low banded matrix
    size_t mujac;           // (0)  <= n upper banded matrix
    size_t mxordn;          // (12) maximalen Ordnung non stiff (max = 12)
    size_t mxords;          // (5)  maximalen Ordnung siff (max=5)
  };

  struct SIntegratorRADInp      // Parameter expliziter RungeKutta
  {
    size_t ijac;            // (1)  0/1 Jakobimatrix zu berechnen
    ...
    size_t m2;              // (0)  Anzahl zweite Ableitung m1 sollte m2
  ...
  public:
    ...
    size_t        nmax;        // 100000 maximal number of allowed steps, wenn 0, default 100000
  ...
  protected:
    ...
    size_t          N;                // Anzahl Zustände
    size_t          Nmax;             // maximal zulässige Anzahl von steps
    ...
    double         Hnext;            // build Hnext = X + H - Xend; for next step
    ...
    size_t       Norder;           // Ordnung

    size_t       Nfcn;             // Anzahl Funktionsaufrufe
    size_t       Nstep;            // aktuelle Stepzahl
    size_t       NstepMax;         // maximale Anzahl Steps pro Aufruf
    size_t       NstepAccept;      // Anzahl angenommener Steps dabei
    size_t       Njac;             // Anzahl Jacobiaufrufe
  ...
  public:
    ...
    //virtual okay_t init(CIntegratorBaseInp &inp) = 0; /* Prototype init */
    ...
    size_t nfcnRead(void){ return Nfcn; }
    size_t nstepRead(void){ return Nstep; }
    size_t naccptRead(void){ return NstepAccept; }
    double hRead (void){ return H; }
    double xRead (void){ return X; }
    CVectorD &yRead (void){return Y;}
    size_t norderRead(void){ return Norder; }
    size_t methRead(void){ return Meth; }
    size_t njacRead(void){ return Njac; }
