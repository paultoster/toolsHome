...
static char     NumericErrMess[BUFSIZ];
static status_t NumericStatus = 0;
char *pNumericErrMess = NumericErrMess;

/*===========================================================*/
status_t NumericInterp1D(uint8_t iflag
                       ,double *x   
                       ,double *y
                       ,uint32_t nxy
                       ,double x0
      ...
      if(iflag == 1) /* Initialisierung */
      {
        for(i=0;i<nxy-1;i++)
        {
            if( (x[i+1]-x[i]) < (double)EPSILON ) {
                ...
                sprintf_s(NumericErrMess,BUFSIZ,"Error NumericInterp1D: x-vector is not monoton increasing !!!");
#else
                sprintf(NumericErrMess,"Error NumericInterp1D: x-vector is not monoton increasing !!!");
#endif
                NumericStatus = NOT_OKAY;
                return NOT_OKAY;
            }
        }
      }
...
/*==============================================================*/
status_t NumericInterp1DTimeOut(double *xin
  , double *yin
  , uint32_t nxyin
  , double *xout
  , double *yout
  ...
  , double   dxout)
{
  ...
  // timeout mit flag festlegen
  if (dxout > 0.0) flag_dxout = 1;
  else              flag_dxout = 0;
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess, BUFSIZ, "Error NumericInterp1DTimeOut: nxyin == 0 !!!");
#else
    sprintf(NumericErrMess, "Error NumericInterp1DTimeOut: nxyin == 0 !!!");
#endif
    NumericStatus = NOT_OKAY;
    return NOT_OKAY;
  }
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess, BUFSIZ, "Error NumericInterp1DTimeOut: nxyout == 0 !!!");
#else
    sprintf(NumericErrMess, "Error NumericInterp1DTimeOut: nxyout == 0 !!!");
#endif
    NumericStatus = NOT_OKAY;
    return NOT_OKAY;
  }

  // init (prüft monotonie)
  if (NumericInterp1D(1, xin, yin, nxyin, xout[iact], &yout[iact], &iact, iord, iabl) != OKAY) return NOT_OKAY;

  for (i = 0; i<nxyout; i++)
  {
    // vor dem Wertebereich
    if (xout[i] < xin[0])
    {
      if (flag_dxout) // time_out
      {
        yout[i] = 0.0;
      }
      else
      {
        if (!extrap)
        {
          if (iabl == 0) yout[i] = yin[0];  // Wert
          else            yout[i] = 0.0;    // Steigung
        }
        else
        {
          NumericInterp1D(0, xin, yin, nxyin, xout[i], &yout[i], &iact, iord, iabl);
        }
      }
    }
    // nach dem Wertebereich
    else if (xout[i] >= xin[nxyin - 1])
    {
      if (flag_dxout && ((xout[i] - xin[nxyin - 1]) > dxout)) // time_out
      {
        yout[i] = 0.0;
      }
      else
      {
        if (!extrap)
        {
          if (iabl == 0) yout[i] = yin[nxyin - 1];  // Wert
          else            yout[i] = 0.0;          // Steigung
        }
        else
        {
    ...
    else
    {
      NumericInterp1D(0, xin, yin, nxyin, xout[i], &yout[i], &iact, iord, iabl);
      // time_out
      if (flag_dxout && ((xout[i] - xin[iact])>dxout)) yout[i] = 0.0;
    }
  }
  return OKAY;
}
/*==============================================================*/
void NumericInterp2D_search_index(size_t *piLeft, size_t *piRght,
...
/*==============================================================*/
int NumericInterp2D(uint8_t iflag,double *xvec,size_t nx,double *yvec,size_t ny,
...
********************************************************************/
{
    ...
    NumericInterp2D_search_index(&index_x1,&index_x2,x0,xvec,nx);
    NumericInterp2D_search_index(&index_y1,&index_y2,y0,yvec,ny);
...
/*==============================================================*/
void NumericStepSet(SNumericStep *s,double xa,double xb,double ya,double yb) 
{
...
void NumericStepCalc(SNumericStep *s,double x) 
{
...
/*===========================================================*/
double NumericAbs(double x)
{
  if( x >= 0.0 ) return x;
  else           return x*(-1.0);
}
/*===========================================================*/
sint8_t NumericSgn(double x)
{
  if( x >= 0.0 ) return 1;
  else           return -1;
}
/*****************************************************************************
  @fn                   NumericLim1(double *pVal, double *pLimUp, double *pLimLow)
...
*****************************************************************************/
double NumericLim(double val, double limUp, double limLow)
{
...
/*****************************************************************************
  @fn              double NumericLim1(double Val, double Lim)
  @description     limits Value "val" between Lim1 and Lim2
...
double NumericLim1(double val, double lim)
{
  double lim0 = lim * (-1.f);
...
/*===========================================================*/
double NumericATan2(double y,double x)
{
    double dum;
...
//  m = sqrt(a^2+b^2)
//===============================================
double NumericSqrt2(double a, double b) 
{
    return sqrt(a*a+b*b);
}
/***********************************************************************
...
Vector_t NewVector(uarray r)
{
	...
	a = (struct array_header *)
	    NumericAllocate(sizeof(struct array_header) + r * sizeof(double), char);
  ...
  a->link  = 0;
	...
	if(HEADER(v) != (struct array_header *) a ||
	   GET_NDIMS(v) != 1 || GET_NROWS(v) != r || GET_NCOLS(v) != 1) {
        ...
        sprintf_s(NumericErrMess,BUFSIZ,"NewRowVector error: v=%p  D:%d,%d  R:%d,%d  C:%d,%d\n", (void *)v,    GET_NDIMS(v), 1,  GET_NROWS(v), r, GET_NCOLS(v), 1);
#else
	    	sprintf(NumericErrMess,"NewRowVector error: v=%x H: %x,%x  D:%d,%d  R:%d,%d  C:%d,%d\n", v,  HEADER(v), a,  GET_NDIMS(v), 1,  GET_NROWS(v), r, GET_NCOLS(v), 1);
#endif
        NumericStatus = NOT_OKAY;
        return NULL;
	    }
...
Matrix_t NewMatrix(uarray r,uarray c)
{
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"NewCollVector error: v=%p D:%d,%d  R:%d,%d  C:%d,%d\n", (void *)m,    GET_NDIMS(m), 2,  GET_NROWS(m), r, GET_NCOLS(m), c);
#else
	  sprintf(NumericErrMess,"NewCollVector error: v=%x H: %x,%x  D:%d,%d  R:%d,%d  C:%d,%d\n", m,  HEADER(m), a,  GET_NDIMS(m), 2,  GET_NROWS(m), r, GET_NCOLS(m), c);
#endif
    NumericStatus = NOT_OKAY;
    return NULL;
	}
...
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"InnerProduct %d x %d ", GET_NROWS(v1), GET_NROWS(v2));
#else
		sprintf(NumericErrMess,"InnerProduct %d x %d ", GET_NROWS(v1), GET_NROWS(v2));
#endif
        NumericStatus = NOT_OKAY;
        return 0.0;
	    }
...
void MatrixMultiply(register Matrix_t m1,register Matrix_t  m2,register Matrix_t  result)
{
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MatrixMultiply: Can't multiply %dx%d and %dx%d matrices",
			GET_NROWS(m1), GET_NCOLS(m1), GET_NROWS(m2), GET_NCOLS(m2));
#else
	  sprintf(NumericErrMess,"MatrixMultiply: Can't multiply %dx%d and %dx%d matrices",
			GET_NROWS(m1), GET_NCOLS(m1), GET_NROWS(m2), GET_NCOLS(m2));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MatrixMultiply: %dx%d times %dx%d does not give %dx%d product",
			GET_NROWS(m1), GET_NCOLS(m1), GET_NROWS(m2), GET_NCOLS(m2),
			GET_NROWS(result), GET_NCOLS(result));
#else
		sprintf(NumericErrMess,"MatrixMultiply: %dx%d times %dx%d does not give %dx%d product",
			GET_NROWS(m1), GET_NCOLS(m1), GET_NROWS(m2), GET_NCOLS(m2),
			GET_NROWS(result), GET_NCOLS(result));
#endif
    NumericStatus = NOT_OKAY;
...
void MatrixTimesVector(Matrix_t m,Vector_t v,Vector_t vres)
{
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MatrixTimesVector: %dx%d mat time %d vector does not fit",
			GET_NROWS(m), GET_NCOLS(m), GET_NROWS(v));
#else
		sprintf(NumericErrMess,"MatrixTimesVector: %dx%d mat time %d vector does not fit",
			GET_NROWS(m), GET_NCOLS(m), GET_NROWS(v));
#endif
    NumericStatus = NOT_OKAY;
		return;
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MatrixTimesVector: %d res-vector = %dx%d mat time %d vector does not fit",
			GET_NROWS(vres),GET_NROWS(m), GET_NCOLS(m), GET_NROWS(v));
#else
		sprintf(NumericErrMess,"MatrixTimesVector: %d res-vector = %dx%d mat time %d vector does not fit",
			GET_NROWS(vres),GET_NROWS(m), GET_NCOLS(m), GET_NROWS(v));
#endif
        NumericStatus = NOT_OKAY;
		return;
...
void VectorTimesMatrix(Vector_t v,Matrix_t m,Vector_t vres)
{
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"VectorTimesMatrix: Can't multiply %d vector by %dx%d",
			GET_NROWS(v), GET_NROWS(m), GET_NCOLS(m));
#else
		sprintf(NumericErrMess,"VectorTimesMatrix: Can't multiply %d vector by %dx%d",
			GET_NROWS(v), GET_NROWS(m), GET_NCOLS(m));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"VectorTimesMatrix: %d vector times %dx%d mat does not fit in %d product" ,
			GET_NROWS(v), GET_NROWS(m), GET_NCOLS(m), GET_NROWS(vres));
#else
		sprintf(NumericErrMess,"VectorTimesMatrix: %d vector times %dx%d mat does not fit in %d product" ,
			GET_NROWS(v), GET_NROWS(m), GET_NCOLS(m), GET_NROWS(vres));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
void ScalarTimesVector(double s,Vector_t v,Vector_t vres)
{
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"ScalarTimesVector: result wrong size (%d!=%d)",
			GET_NROWS(v), GET_NROWS(vres));
#else
		sprintf(NumericErrMess,"ScalarTimesVector: result wrong size (%d!=%d)",
			GET_NROWS(v), GET_NROWS(vres));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
void ScalarTimesMatrix(double s,Matrix_t m,Matrix_t mres)
{
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"ScalarTimesMatrix: result wrong size (%d!=%d)or(%d!=%d)",
			GET_NROWS(m), GET_NROWS(mres),
			GET_NCOLS(m), GET_NCOLS(mres));
#else
		sprintf(NumericErrMess,"ScalarTimesMatrix: result wrong size (%d!=%d)or(%d!=%d)",
			GET_NROWS(m), GET_NROWS(mres),
			GET_NCOLS(m), GET_NCOLS(mres));
#endif
    NumericStatus = NOT_OKAY;
...
double QuadraticForm(register Vector_t v,register Matrix_t  m)
{
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"QuadraticForm: bad matrix size (%dx%d not %dx%d)",
			GET_NROWS(m), GET_NCOLS(m), n, n);
#else
		sprintf(NumericErrMess,"QuadraticForm: bad matrix size (%dx%d not %dx%d)",
			GET_NROWS(m), GET_NCOLS(m), n, n);
#endif
    NumericStatus = NOT_OKAY;
    return 0;
...
double InvertMatrix(Matrix_t ym,Matrix_t rm)
{
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"InvertMatrix: not square");
#else
		sprintf(NumericErrMess,"InvertMatrix: not square");
#endif
    NumericStatus = NOT_OKAY;
    return 0;
	}
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"InvertMatrix: result wrong size");
#else
		sprintf(NumericErrMess,"InvertMatrix: result wrong size");
#endif
    NumericStatus = NOT_OKAY;
    return 0;
	}
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"InvertMatrix: PERMBUFSIZE");
#else
		sprintf(NumericErrMess,"InvertMatrix: PERMBUFSIZE");
#endif
    NumericStatus = NOT_OKAY;
    return 0;
	}
...
void VectorAdd(Vector_t va,Vector_t vb, Vector_t vres)
{
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"VectorAdd: result wrong size (dim(va)=%d !=dim(vb)=%d )",
			GET_NROWS(va), GET_NROWS(vb));
#else
		sprintf(NumericErrMess,"VectorAdd: result wrong size (dim(va)=%d !=dim(vb)=%d )",
			GET_NROWS(va), GET_NROWS(vb));
#endif
    NumericStatus = NOT_OKAY;
		return;
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"VectorAdd: result wrong size (dim(va)=%d !=dim(vres)=%d )",
			GET_NROWS(va), GET_NROWS(vres));
#else
		sprintf(NumericErrMess,"VectorAdd: result wrong size (dim(va)=%d !=dim(vres)=%d )",
			GET_NROWS(va), GET_NROWS(vres));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
void VectorAddScalar(double s,Vector_t v,Vector_t vres)
{
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"VectorAddScalar: result wrong size (%d!=%d)",
			GET_NROWS(v), GET_NROWS(vres));
#else
		sprintf(NumericErrMess,"VectorAddScalar: result wrong size (%d!=%d)",
			GET_NROWS(v), GET_NROWS(vres));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
void MatrixAddScalar(double s,Matrix_t ma, Matrix_t mres) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MatrixAddScalar: result wrong size (dim(mres)=(%dx%d) !=dim(ma)=(%dx%d) )",
			GET_NROWS(mres),GET_NCOLS(mres), GET_NROWS(ma),GET_NCOLS(ma));
#else
		sprintf(NumericErrMess,"MatrixAddScalar: result wrong size (dim(mres)=(%dx%d) !=dim(ma)=(%dx%d) )",
			GET_NROWS(mres),GET_NCOLS(mres), GET_NROWS(ma),GET_NCOLS(ma));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
void VectorSub(Vector_t va,Vector_t vb, Vector_t vres)
{
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"VectorSub: result wrong size (dim(va)=%d !=dim(vb)=%d )",
			GET_NROWS(va), GET_NROWS(vb));
#else
		sprintf(NumericErrMess,"VectorSub: result wrong size (dim(va)=%d !=dim(vb)=%d )",
			GET_NROWS(va), GET_NROWS(vb));
#endif
    NumericStatus = NOT_OKAY;
		return;
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"VectorSub: result wrong size (dim(va)=%d !=dim(vres)=%d )",
			GET_NROWS(va), GET_NROWS(vres));
#else
		sprintf(NumericErrMess,"VectorSub: result wrong size (dim(va)=%d !=dim(vres)=%d )",
			GET_NROWS(va), GET_NROWS(vres));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
void VectorMultAdd(double s,Vector_t vadd, Vector_t vio) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"VectorMultAdd: result wrong size (dim(vio)=%d !=dim(vadd)=%d )",
			GET_NROWS(vio), GET_NROWS(vadd));
#else
		sprintf(NumericErrMess,"VectorMultAdd: result wrong size (dim(vio)=%d !=dim(vadd)=%d )",
			GET_NROWS(vio), GET_NROWS(vadd));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
void VectorMultSub(double s,Vector_t vsub, Vector_t vio) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"VectorMultSub: result wrong size (dim(vio)=%d !=dim(vsub)=%d )",
			GET_NROWS(vio), GET_NROWS(vsub));
#else
		sprintf(NumericErrMess,"VectorMultSub: result wrong size (dim(vio)=%d !=dim(vsub)=%d )",
			GET_NROWS(vio), GET_NROWS(vsub));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
void MatrixMultAdd(double s,Matrix_t madd, Matrix_t mio) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MatrixMultAdd: result wrong size (dim(mio)=(%dx%d) !=dim(madd)=(%dx%d) )",
			GET_NROWS(mio),GET_NCOLS(mio), GET_NROWS(madd),GET_NCOLS(madd));
#else
		sprintf(NumericErrMess,"MatrixMultAdd: result wrong size (dim(mio)=(%dx%d) !=dim(madd)=(%dx%d) )",
			GET_NROWS(mio),GET_NCOLS(mio), GET_NROWS(madd),GET_NCOLS(madd));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
void MatrixMultSub(double s,Matrix_t msub, Matrix_t mio) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MatrixMultSub: result wrong size (dim(mio)=(%dx%d) !=dim(madd)=(%dx%d) )",
			GET_NROWS(mio),GET_NCOLS(mio), GET_NROWS(msub),GET_NCOLS(msub));
#else
		sprintf(NumericErrMess,"MatrixMultSub: result wrong size (dim(mio)=(%dx%d) !=dim(madd)=(%dx%d) )",
			GET_NROWS(mio),GET_NCOLS(mio), GET_NROWS(msub),GET_NCOLS(msub));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
void UnifyVector(Vector_t v, Vector_t e) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"UnifyVector: result wrong size v(%d) != e(%d)",
			GET_NROWS(v), GET_NROWS(e));
#else
	  sprintf(NumericErrMess,"UnifyVector: result wrong size v(%d) != e(%d)",
			GET_NROWS(v), GET_NROWS(e));
#endif
    NumericStatus = NOT_OKAY;
		return;
...
void MakeTildeMatrix(Vector_t v,Matrix_t mres) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MakeTildeMatrix: Vektorwrong size v(%d) != 3",
			GET_NROWS(v));
#else
		sprintf(NumericErrMess,"MakeTildeMatrix: Vektorwrong size v(%d) != 3",
			GET_NROWS(v));
#endif
    NumericStatus = NOT_OKAY;
		return;
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MakeTildeMatrix: result wrong size mres(%d,%d) != 3x3",
			GET_NROWS(mres),
			GET_NCOLS(mres));
#else
		sprintf(NumericErrMess,"MakeTildeMatrix: result wrong size mres(%d,%d) != 3x3",
			GET_NROWS(mres),
			GET_NCOLS(mres));
#endif
    NumericStatus = NOT_OKAY;
...
void MakeTransponeTildeMatrixMultTildeMatrix(Vector_t v,Matrix_t mres) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MakeTransponeTildeMatrixMultTildeMatrix: Vektorwrong size v(%d) != 3",
			GET_NROWS(v));
#else
		sprintf(NumericErrMess,"MakeTransponeTildeMatrixMultTildeMatrix: Vektorwrong size v(%d) != 3",
			GET_NROWS(v));
#endif
    NumericStatus = NOT_OKAY;
		return;
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MakeTransponeTildeMatrixMultTildeMatrix: result wrong size mres(%d,%d) != 3x3",
			GET_NROWS(mres),
			GET_NCOLS(mres));
#else
		sprintf(NumericErrMess,"MakeTransponeTildeMatrixMultTildeMatrix: result wrong size mres(%d,%d) != 3x3",
			GET_NROWS(mres),
			GET_NCOLS(mres));
#endif
    NumericStatus = NOT_OKAY;
...
void MakeVectorMultTransponeVektor(Vector_t va,Vector_t vtotrans, Matrix_t mres) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MakeVectorMultTransponeVektor: result wrong size");
#else
		sprintf(NumericErrMess,"MakeVectorMultTransponeVektor: result wrong size");
#endif
    NumericStatus = NOT_OKAY;
    return;
	}
...
void MatrixAdd(Matrix_t ma,Matrix_t mb,Matrix_t mres) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MatrixAdd: result wrong size ma(%d x %d) != mres(%d x %d)",
			GET_NROWS(ma), GET_NCOLS(ma),
			GET_NROWS(mres), GET_NCOLS(mres));
#else
		sprintf(NumericErrMess,"MatrixAdd: result wrong size ma(%d x %d) != mres(%d x %d)",
			GET_NROWS(ma), GET_NCOLS(ma),
			GET_NROWS(mres), GET_NCOLS(mres));
#endif
    NumericStatus = NOT_OKAY;
...
void MatrixSub(Matrix_t ma,Matrix_t mb,Matrix_t mres) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"MatrixSub: result wrong size ma(%d x %d) != mres(%d x %d)",
			GET_NROWS(ma), GET_NCOLS(ma),
			GET_NROWS(mres), GET_NCOLS(mres));
#else
		sprintf(NumericErrMess,"MatrixSub: result wrong size ma(%d x %d) != mres(%d x %d)",
			GET_NROWS(ma), GET_NCOLS(ma),
			GET_NROWS(mres), GET_NCOLS(mres));
#endif
    NumericStatus = NOT_OKAY;
...
void PutMatrixToMatrix(Matrix_t m,Matrix_t M,uarray irow,uarray icol) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"PutMatrixToMatrix: size m(%d x %d) doesn't fit to M(%d x %d) at pos irow=%d,icol=%d",
			GET_NROWS(m), GET_NCOLS(m),
			GET_NROWS(M), GET_NCOLS(M),
            irow,icol);
#else
		sprintf(NumericErrMess,"PutMatrixToMatrix: size m(%d x %d) doesn't fit to M(%d x %d) at pos irow=%d,icol=%d",
			GET_NROWS(m), GET_NCOLS(m),
			GET_NROWS(M), GET_NCOLS(M),
            irow,icol);
#endif
...
void PutVectorToMatrix(Vector_t v, Matrix_t M, uarray irow, uarray icol) {
	...
	if(irow+GET_NROWS(v) > GET_NROWS(M)  || 
	   icol+1 > GET_NCOLS(M)) {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"PutVectorToMatrix: size v(%d) doesn't fit to M(%d x %d) at pos irow=%d,icol=%d",
			GET_NROWS(v),
			GET_NROWS(M), GET_NCOLS(M),
            irow,icol);
#else
		sprintf(NumericErrMess,"PutVectorToMatrix: size v(%d) doesn't fit to M(%d x %d) at pos irow=%d,icol=%l",
			GET_NROWS(v),
			GET_NROWS(M), GET_NCOLS(M),
            irow,icol);
#endif
...
void PutVectorToVector(Vector_t v, Vector_t vres, uarray irow) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"PutVectorxToVector: size v(%d) doesn't fit to vres(%d) at pos irow=%d",
			GET_NROWS(v),
			GET_NROWS(vres),
            irow);
#else
		sprintf(NumericErrMess,"PutVectorxToVector: size v(%d) doesn't fit to vres(%d) at pos irow=%d",
			GET_NROWS(v),
			GET_NROWS(vres),
            irow);
#endif
...
void PutMatrixToMatrixMax(Matrix_t m, Matrix_t M, uarray nrowmax, uarray ncolmax) {
  ...
  {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"TransponeMatrix: result wrong size m(%d x %d) could not be transponed to mres(%d x %d)",
			GET_NROWS(m), GET_NCOLS(m),
			GET_NROWS(mres), GET_NCOLS(mres));
#else
		sprintf(NumericErrMess,"TransponeMatrix: result wrong size m(%d x %d) could not be transponed to mres(%d x %d)",
			GET_NROWS(m), GET_NCOLS(m),
			GET_NROWS(mres), GET_NCOLS(mres));
#endif
    NumericStatus = NOT_OKAY;
...
// use NumericAllocate(n, type)
//=================================================================
//=================================================================

void *NumericAllocateFunc(uint32_t nitems, uint32_t itemsize, char *tname)
{
    ...
    {
#if _MSC_VER > MSC_VER_BIS_VS2005
    sprintf_s(NumericErrMess,BUFSIZ,"Error NumericAllocatFunc: Can't get mem for %d %s's (each %d bytes, %d total bytes)",
		nitems, tname, itemsize, bytes);
#else
       sprintf(NumericErrMess,"Error NumericAllocatFunc: Can't get mem for %d %s's (each %d bytes, %d total bytes)",
		nitems, tname, itemsize, bytes);
...
#endif
double NumericDmaxnorm(sint32_t n, double *v, double *w)
{
...
/* DECK DBNORM */
double NumericDbnorm(sint32_t n, double *a, sint32_t nra, sint32_t ml, 
...
double NumericDfnorm(sint32_t n, double *a, double *w)
{
    /* System generated locals */
    sint32_t a_dim1, a_offset, i__1, i__2;
    double ret_val, d__1, d__2;
...
#else
int NumericDewset(sint32_t n, sint32_t itol, double *rtol, 
...
int NumericDcfode(sint32_t meth, double *elco, double *tesco)
{
    /* System generated locals */
    sint32_t i__1;
...
int NumericDgbfa(double *abd, sint32_t lda, sint32_t n, 
...
/* Computing MIN */
	...
	lm = MIN(i__2,i__3);
	i__2 = lm + 1;
	l = NumericIdamax(i__2, &abd[m + k * abd_dim1], 1) + m - 1;
	ipvt[k] = l + k - m;
...
/*           COMPUTE MULTIPLIERS */
	...
	NumericDscal(lm, &t, &abd[m + 1 + k * abd_dim1], 1);
...
/* Computing MAX */
	...
	for (j = kp1; j <= i__2; ++j) {
	    ...
	    abd[mm + j * abd_dim1] = t;
L70:
	    NumericDaxpy(lm, &t, &abd[m + 1 + k * abd_dim1], 1, &abd[mm + 1 + 
		    j * abd_dim1], 1);
/* L80: */
	}
L90:
...
int NumericDscal(sint32_t n, double *da, double *dx, sint32_t incx)
{
    /* System generated locals */
    sint32_t i__1;
...
int NumericDaxpy(sint32_t n, double *da, double *dx, 
...
sint32_t NumericIdamax(sint32_t n, double *dx, sint32_t incx)
{
    /* System generated locals */
...
int NumericDgefa(double *a, sint32_t lda, sint32_t n, sint32_t *ipvt, sint32_t *info)
{
    /* System generated locals */
...
/*        FIND L = PIVOT INDEX */
	...
	i__2 = n - k + 1;
	l = NumericIdamax(i__2, &a[k + k * a_dim1], 1) + k - 1;
	ipvt[k] = l;
...
/*           COMPUTE MULTIPLIERS */
	...
	t = -1. / a[k + k * a_dim1];
	i__2 = n - k;
	NumericDscal(i__2, &t, &a[k + 1 + k * a_dim1], 1);
...
L20:
	    i__3 = n - k;
	    NumericDaxpy(i__3, &t, &a[k + 1 + k * a_dim1], 1, &a[k + 1 + j * 
		    a_dim1], 1);
/* L30: */
	}
	goto L50;
...
int NumericDgbsl(double *abd, sint32_t lda, sint32_t n, 
...
/* Computing MIN */
	...
	b[k] = t;
L10:
	NumericDaxpy(lm, &t, &abd[m + 1 + k * abd_dim1], 1, &b[k + 1], 1);
/* L20: */
    }
L30:
...
/*        NOW SOLVE  U*X = Y */
	...
	lb = k - lm;
	t = -b[k];
	NumericDaxpy(lm, &t, &abd[la + k * abd_dim1], 1, &b[lb], 1);
/* L40: */
    }
    goto L100;
L50:
...
/*        FIRST SOLVE  TRANS(U)*Y = B */
	...
	la = m - lm;
	lb = k - lm;
	t = NumericDdot(lm, &abd[la + k * abd_dim1], 1, &b[lb], 1);
...
/* Computing MIN */
	i__2 = ml, i__3 = n - k;
	lm = MIN(i__2,i__3);
	b[k] += NumericDdot(lm, &abd[m + 1 + k * abd_dim1], 1, &b[k + 1], 1);
	l = ipvt[k];
	if (l == k) {
	    goto L70;
	}
...
double NumericDdot(sint32_t n, double *dx, sint32_t incx, double *dy, sint32_t incy)
{
    /* System generated locals */
    sint32_t i__1, i__2;
    double ret_val;
...
int NumericDgesl(double *a, sint32_t lda, sint32_t n, sint32_t *ipvt, double *b, sint32_t job)
{
    /* System generated locals */
    sint32_t a_dim1, a_offset, i__1, i__2;
...
L10:
	i__2 = n - k;
	NumericDaxpy(i__2, &t, &a[k + 1 + k * a_dim1], 1, &b[k + 1], 1);
/* L20: */
    }
L30:
...
/*        NOW SOLVE  U*X = Y */
	...
	t = -b[k];
	i__2 = k - 1;
	NumericDaxpy(i__2, &t, &a[k * a_dim1 + 1], 1, &b[1], 1);
/* L40: */
    }
    goto L100;
L50:
...
/*        FIRST SOLVE  TRANS(U)*Y = B */
    ...
    for (k = 1; k <= i__1; ++k) {
	i__2 = k - 1;
	t = NumericDdot(i__2, &a[k * a_dim1 + 1], 1, &b[1], 1);
...
/*        NOW SOLVE TRANS(L)*X = Y */
	...
	k = n - kb;
	i__2 = n - k;
	b[k] += NumericDdot(i__2, &a[k + 1 + k * a_dim1], 1, &b[k + 1], 1);
	l = ipvt[k];
	if (l == k) {
	    goto L70;
	}
...
double NumericPowDi(double x, sint32_t n) {
...
/*-------------------------------------------------------------------------
void   FilterDigPar(SNumericFiltDig *ps, uint8_t n,double *pa, double *pb);
void   FilterDigInit(SNumericFiltDig *ps, uint8_t n,double *px, double *py);
double FilterDigCalc(SNumericFiltDig *ps, double xact);
...
n=2  a0=>a[0],a1=>a[1],a2=>a[2] => double a[n+1];
  ...
  y(i) = -- ( sum(x(i-j)*bj) - sum(y(i-j)*aj) )
         a0   j=0              j=1
-------------------------------------------------------------------------*/
//#define NUMERIC_MAX_FILT_ORDER    5
//typedef struct tag_SNumericFiltDig {
//    uint8_t        n;
//    double       x[NUMERIC_MAX_FILT_ORDER];
//    double       y[NUMERIC_MAX_FILT_ORDER];
//    double       a[NUMERIC_MAX_FILT_ORDER];
//    double       b[NUMERIC_MAX_FILT_ORDER];
//}SNumericFiltDig;
void   NumericFiltDigPar(SNumericFiltDig *ps,double *pa,uint8_t na,double *pb,uint8_t nb)
{
  uint8_t i;
  ps->na = MIN(na,NUMERIC_MAX_FILT_ORDER);
  for(i=0;i<(ps->na+1);i++)
  {
    ps->a[i] = pa[i];
    ps->y[i] = 0.0;
  ...
  ps->nb = MIN(nb,NUMERIC_MAX_FILT_ORDER);
  for(i=0;i<(ps->nb+1);i++)
  {
    ps->b[i] = pb[i];
    ps->x[i] = 0.0;
...
void   NumericFiltDigInit(SNumericFiltDig *ps,double *px, uint8_t nx, double *py, uint8_t ny)
{
  uint8_t i;
  uint8_t n1 = MIN(nx,NUMERIC_MAX_FILT_ORDER);
  for(i=0;i<(n1+1);i++)
  {
    ps->x[i] = px[i];
  }
  n1 = MIN(ny,NUMERIC_MAX_FILT_ORDER);
  for(i=0;i<(n1+1);i++)
  {
    ps->y[i] = py[i];
  }
}
double NumericFiltDigCalc(SNumericFiltDig *ps, double xact)
{
  uint8_t  i;
  for(i=ps->na;i>0;i--)
  {
...
/*-------------------------------------------------------------------------
void   NumericPt1FiltInit(SNumericFiltDig *ps, double t1, double dt)
double FilterDigCalc(SNumericFiltDig *ps, double xact);

Init PT1-Filter for digital-Filter use FilterDigCalc() for Calc
-------------------------------------------------------------------------*/
void NumericPt1FiltInit(SNumericFiltDig *ps, double t1, double dt, double y0)
{
  ps->na = 1;
  ps->nb = 1;
  ps->a[0] = 1.0;
